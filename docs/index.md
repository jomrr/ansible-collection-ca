# CA Collection Module Documentation

This directory documents the collection Ansible modules and internal helpers
ported from the CA role.

The collection implements a managed two-tier CA with declarative authorities,
certificates, chains, CRLs, bundles, FritzBox deployment, and a non-secret CA
inventory state. The public modules live in `plugins/modules/`; shared implementation
code lives in `plugins/module_utils/` and `plugins/filter/`.

## Public Module Flow

Call the public modules in this order:

1. `jomrr.ca.authority` creates root and issuing CA certificates.
2. `jomrr.ca.chain` derives issuing CA chain files from the generated CA certificates.
3. `jomrr.ca.certificate_batch` dispatches the declarative `ca_certificates` list to
   ...
   `csr_content`. `jomrr.ca.certificate` uses the same engine for one certificate.
4. `jomrr.ca.fritzbox_deploy` can deploy a generated FritzBox bundle to FRITZ!OS.
5. `jomrr.ca.crl` resolves declared revocations and creates PEM/DER CRL exports for
   every CA.
6. Optional publish tasks use `jomrr.ca.publish_archive` to build deterministic
   archives and unpack CA certificates, issuing chains, and CRLs to configured
   AIA/CDP target webroots.

`jomrr.ca.authority`, `jomrr.ca.certificate_batch`, `jomrr.ca.certificate`, and `jomrr.ca.crl` update
internal CA inventory fragments. When `ca_name` is set, the fragments are
composed into `<base_dir>/inventory/ca-inventory.json`.

## Public Modules

- [jomrr.ca.authority](authority.md)
- [jomrr.ca.chain](chain.md)
- [jomrr.ca.certificate](certificate.md)
- [jomrr.ca.certificate_batch](certificate_batch.md)
- [jomrr.ca.crl](crl.md)
- [jomrr.ca.fritzbox_deploy](fritzbox_deploy.md)
- [jomrr.ca.publish_archive](publish_archive.md)
...
- [ca_file](file.md)
- [ca_certificate_engine](certificate_engine.md)
- [ca_inventory](inventory.md)
- [ca_profiles](profiles.md)
- [ca_renewal](renewal.md)
- [ca_serial](serial.md)
- [ca_text](text.md)
- [ca_time](time.md)
- [ca_validation](validation.md)
- [ca_x509](x509.md)
- [plugins/filter/authority_map.py](authority_map.md)
...
the module runs. Setting `renew_before_days: 0` disables advance renewal. The
policy
...
The role passes the selected authority list to `jomrr.ca.crl` as
`revoked_certificates`. Revocations are resolved by `jomrr.ca.crl` when CRLs are
